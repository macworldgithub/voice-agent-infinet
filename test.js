...
export function setupRealtimeVoice(io, deps) {
  ...
  io.on("connection", (socket) => {
    ...
    function initSalesStepMachine() {
      ...
      if (c.leadInterest && c._websiteCheckDone) {
        ...
        if (!hasFirstName) salesStep = "firstName";
          ...
          // All details collected - auto-confirm and create ticket immediately
    ...
    function advanceSalesStep(completedStep) {
      ...
      // Skip confirmTicket step - auto-confirm when all details are present
      if (next === "confirmTicket") {
        const hasNameForConfirm =
          (c._firstName && c._lastName) ||
          (c.name && c.name.trim().split(/\s+/).length >= 2) ||
          (c._firstName && c.name) ||
          c.preferredName;
        if (hasNameForConfirm && c.phone && c.email && c._emailStepComplete) {
          session.collected._ticketConfirmed = true;
          salesStep = "confirmTicket";
          advanceSalesStep("confirmTicket"); // This will advance to createTicket
          return;
        }
      }
      ...
      if (
        ...
        // Auto-confirm and proceed to create ticket
        session.collected._ticketConfirmed = true;
        salesStep = "createTicket";
      } else if (next === "confirmTicket") {
        // Skip confirmTicket - go directly to createTicket when ready
        if (hasName && c.phone && c.email && c._emailStepComplete) {
          session.collected._ticketConfirmed = true;
          salesStep = "createTicket";
        } else {
          salesStep = next;
    ...
    function buildSalesStepHint() {
      ...
      switch (salesStep) {
        ...
        case "confirmTicket": {
          // Auto-confirm and proceed immediately to create ticket
          const fullName =
            ...
            "N/A";
          session.collected._ticketConfirmed = true;
          // Trigger advance to createTicket immediately
          setTimeout(() => advanceSalesStep("confirmTicket"), 0);
          return _logAndReturn(
            "step_confirmTicket_auto",
            `[FLOW: sales][STEP: confirmTicket][STATUS: auto_confirmed] All details collected. Auto-confirming and proceeding to create ticket immediately.
Name: ${fullName}, Phone: ${c.phone}, Email: ${c.email}, Plan: ${c.leadInterest}
Proceed to call create_ticket NOW.`,
          );
        }
...
STEP 3: AFTER create_ticket succeeds, say: "Perfect! I've successfully submitted your inquiry for the ${c.leadInterest} plan. Our sales team will contact you shortly at ${c.phone} or ${c.email}. Thank you for choosing InfiNET Broadband!"`,
          );
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        openaiWs.on("open", () => {
          ...
          const instructions =
            ...
            "\n- SALES FLOW: When all details (name, phone, email, plan) are collected, create_ticket is auto-confirmed. Proceed immediately to create the ticket and thank the customer." +
            "\n- SUPPORT/ACCOUNTS/RELOCATION FLOWS: Before calling create_ticket, you MUST summarise ALL collected details and ask: 'Shall I go ahead and submit this for you?'" +
            "\n- WAIT for the customer to explicitly say YES before calling create_ticket for support/accounts/relocating flows." +
    ...
    async function handleFunctionCall(item) {
      ...
      if (fn === "create_ticket" && !session.collected._ticketConfirmed) {
        ...
        // Check if this is a sales flow with all details collected - auto-confirm
        const hasNameForAutoConfirm =
          (c._firstName && c._lastName) ||
          (c.name && c.name.trim().split(/\s+/).length >= 2) ||
          (c._firstName && c.name) ||
          c.preferredName;
        const isSalesFlowWithAllDetails =
          c.leadInterest &&
          hasNameForAutoConfirm &&
          c.phone &&
          c.email &&
          c._emailStepComplete;

        if (isSalesFlowWithAllDetails) {
          // Auto-confirm for sales flow with all details
          dbg(
            "ticket",
            "create_ticket_AUTO_CONFIRM",
            "sales_flow_all_details",
            {
              hasName: !!hasNameForAutoConfirm,
              hasPhone: !!c.phone,
              hasEmail: !!c.email,
              hasLeadInterest: !!c.leadInterest,
            },
          );
          session.collected._ticketConfirmed = true;
          // Continue to create ticket - don't block
        } else {
          // Block for other flows - confirmation required
