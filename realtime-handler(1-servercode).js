...
// FIX: Convert email address to letter-by-letter spelling for readback
// e.g., "shaun@bele.ai" → "s-h-a-u-n at b-e-l-e dot a-i"
function emailToSpelling(email) {
  if (!email) return "";
...
export function setupRealtimeVoice(io, deps) {
  ...
  io.on("connection", (socket) => {
    ...
    function startEmailCapture() {
      ...
      // FIX: Only reset email_state if it's not already set with a value needing confirmation
      // If email_state.value is populated (from extraction), preserve it for confirmation
      if (!email_state.value) {
        ...
        dbg("sales", "startEmailCapture", "email_state_preserved", {
          reason: "extracted_awaiting_confirmation",
          value: email_state.value,
        });
        // Set up confirmation state if not yet confirmed
        if (!email_state.is_confirmed) {
          emailCaptureConfirmPending = email_state.value;
    ...
    function handleEmailCaptureTranscript(text) {
      ...
      if (emailCaptureConfirmPending && emailCaptureConfirmAsked) {
        ...
        if (isYes) {
          ...
          // FIX: Only call setEmailValue if the confirmed email differs from current value
          // Otherwise, just call confirmEmail() to set is_confirmed=true without downgrading state
          if (email_state.value !== confirmedEmail) {
          ...
          if (openaiWs?.readyState === WebSocket.OPEN) {
            ...
            // FIX: Include letter-by-letter spelling of email for readback to user
            const emailSpelling = emailToSpelling(confirmedEmail);
            const hint = `The customer has confirmed their email address as ${confirmedEmail}. email_state.is_confirmed=true. createTicketBlockedForEmail=false. IMPORTANT: Before proceeding, read back the email address letter by letter: "${emailSpelling}". Then proceed to next step. If all required details are collected (name, phone, email, plan), call create_ticket NOW.`;
        ...
        if (isNo) {
          ...
          // FIX: Check if user already spelled new email in same utterance
          ...
          if (strippedAfterNo && looksLikeVoiceEmailSpelling(strippedAfterNo)) {
            ...
            emailCaptureBuffer.push(strippedAfterNo);
            const parsedInline = parseVoiceEmail(strippedAfterNo);
            if (parsedInline) {
              setEmailValue(parsedInline);
              emailCaptureConfirmPending = parsedInline;
              emailCaptureConfirmAsked = true;
                  ...
                  JSON.stringify({
                    ...
                    item: {
                      ...
                      content: [
                        {
                          type: "input_text",
                          text: `[SYSTEM_CONTEXT]: Customer corrected email inline. New parse: "${parsedInline}". Read back ONLY exact letters: local="${parsedInline.split("@")[0]}" spell each letter with hyphens, say "at", then domain with "dot". Ask "is that correct?" ONLY.`,
                        },
                      ],
                    },
                  }),
          ...
          // No inline email — ask to spell again
          if (openaiWs?.readyState === WebSocket.OPEN) {
            openaiWs.send(
              JSON.stringify({
                type: "conversation.item.create",
      ...
      if (fn === "extract_call_fields") {
        ...
        if (args.email) {
          ...
          if (
            ...
            // FIX: If we're in email salesStep, set up confirmation-pending state
            // so next user message (e.g., "Yes, this is correct") will be interpreted as confirmation
            if (salesStep === "email") {
              emailCaptureConfirmPending = parsedForExtract;
              emailCaptureConfirmAsked = true;
