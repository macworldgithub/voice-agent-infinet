...
const NATO_MAP = {
  ...
  ay: "a", bee: "b", see: "c", sea: "c", dee: "d", ee: "e", ef: "f", eff: "f",
  gee: "g", aitch: "h", haitch: "h", jay: "j", kay: "k", el: "l", em: "m",
  en: "n", oh: "o", pee: "p", cue: "q", queue: "q", are: "r", ar: "r",
  ess: "s", es: "s", tee: "t", you: "u", vee: "v", double: null, ex: "x",
  why: "y", wye: "y", zee: "z", zed: "z",
  zero: "0", one: "1", two: "2", to: "2", too: "2", three: "3", four: "4",
  for: "4", five: "5", six: "6", seven: "7", eight: "8", nine: "9",
  at: "@", "at sign": "@", dot: ".", period: ".", full: null, stop: ".",
  dash: "-", hyphen: "-", minus: "-", underscore: "_", "under score": "_",
  plus: "+", hash: "#", hashtag: "#", pound: "#",
  com: "com", net: "net", org: "org", edu: "edu", gov: "gov",
  io: "io", co: "co", au: "au", uk: "uk", us: "us", ca: "ca", nz: "nz",
};

const DOMAIN_SHORTCUTS = {
  gmail: "gmail.com", "gmail dot com": "gmail.com", "google mail": "gmail.com",
  yahoo: "yahoo.com", "yahoo dot com": "yahoo.com",
  hotmail: "hotmail.com", "hotmail dot com": "hotmail.com",
  outlook: "outlook.com", "outlook dot com": "outlook.com",
  icloud: "icloud.com", "icloud dot com": "icloud.com",
  live: "live.com", "live dot com": "live.com",
  protonmail: "protonmail.com", "proton mail": "protonmail.com",
  bigpond: "bigpond.com", "bigpond dot com": "bigpond.com",
  optusnet: "optusnet.com.au", tpg: "tpg.com.au", bele: "bele.ai",
};
...
export function setupRealtimeVoice(io, deps) {
  ...
  io.on("connection", (socket) => {
    ...
    function setEmailValue(newEmail) {
      ...
      console.log(`[FLOW: ${session?.collected?.intent || 'unknown'}][STEP: email_capture][STATUS: overwrite][DATA: prev="${prev}" next="${newEmail}" confirmed=false]`);
    }
    ...
    function confirmEmail() {
      ...
      console.log(`[FLOW: ${session?.collected?.intent || 'unknown'}][STEP: email_confirmed][STATUS: success][DATA: email="${email_state.value}" is_confirmed=true session_synced=true]`);
    }
    ...
    function transitionFSM(newState) {
      const prev = fsmState;
      fsmState = newState;
      console.log(`[FLOW: ${session?.collected?.intent || 'unknown'}][STEP: fsm_transition][STATUS: ok][DATA: from="${prev}" to="${newState}"]`);
    ...
    const TimerManager = (() => {
      ...
      function _clearFinalMessage() {
        if (_finalMessageTimer) { clearTimeout(_finalMessageTimer); _finalMessageTimer = null; }
      }
      ...
      return {
        ...
        startSilence(isPackage = false) {
          _clearSilence();
          console.log(`⏱️  [TMgr] startSilence called - isPackage=${isPackage}`);
          ...
          if (
            ...
            console.log(`⏱️  [TMgr] Silence timer suppressed (FSM: ${fsmState})`);
            return;
          }
          if (emailCaptureMode) { console.log(`⏱️  [TMgr] Silence timer suppressed (emailCaptureMode)`); return; }
          if (awaitingStructuredInput) return;
          if (finalMessageLock || session.finalLock) return;
          if (pendingFunctionCalls > 0) return;
          if (elevenLabsStreaming) { console.log(`⏱️  [TMgr] Silence timer suppressed (EL streaming)`); return; }
          if (assistantSpeaking) return;

          const timeoutMs = isPackage ? SILENCE_PACKAGE_MS : SILENCE_NORMAL_MS;
          console.log(`⏱️  [TMgr] Silence timer START: ${timeoutMs / 1000}s (${isPackage ? "package" : "normal"}) [FSM: ${fsmState}]`);
          ...
          _silenceTimer = setTimeout(() => {
            ...
            console.log(`⏰ [TMgr] Silence fired (${timeoutMs / 1000}s) — nudging AI`);
            if (openaiWs?.readyState === WebSocket.OPEN) {
              openaiWs.send(JSON.stringify({
                type: "conversation.item.create",
                item: { type: "message", role: "user", content: [{ type: "input_text", text: nudgeText }] },
              }));
        ...
        startEmailConfirm() {
          _clearEmailConfirm();
          console.log(`⏱️  [TMgr] Email confirm timer START (${EMAIL_CONFIRM_MS / 1000}s)`);
          _emailConfirmTimer = setTimeout(() => {
            ...
            if (openaiWs?.readyState === WebSocket.OPEN) {
              openaiWs.send(JSON.stringify({
                type: "conversation.item.create",
                item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: The customer hasn't responded to the email confirmation. Re-read the email back: "${email_state.value}" — spell each letter individually with hyphens, say "at" for @, say "dot" for full stops. Never say "double X". Ask again if it's correct.` }] },
              }));
              scheduleResponseCreate();
            }
          }, EMAIL_CONFIRM_MS);
        ...
        startWatchdog() {
          ...
          _watchdogTimer = setTimeout(() => {
            ...
            if (!isResponseActive && pendingFunctionCalls === 0) {
              console.warn(`⚠️ [TMgr] Watchdog fired — agent stuck, triggering recovery`);
              if (openaiWs?.readyState === WebSocket.OPEN) {
                openaiWs.send(JSON.stringify({
                  type: "conversation.item.create",
                  item: { type: "message", role: "user", content: [{ type: "input_text", text: "[SYSTEM_CONTEXT]: Please respond immediately to the last user message." }] },
                }));
        ...
        get hasEmailConfirmTimer() { return _emailConfirmTimer !== null; },
      };
    })();
    ...
    function startEmailCapture() {
      if (emailCaptureMode) {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: skipped][DATA: reason=already_active]`);
        return; // idempotent
      }
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: starting][DATA: attempt=1 maxAttempts=${EMAIL_MAX_ATTEMPTS}]`);
      ...
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: active][DATA: buffer=[] blocked=true]`);
    ...
    function resetEmailCapture() {
      ...
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: reset][DATA: mode=false]`);
    }

    function handleEmailCaptureTranscript(text) {
      if (!emailCaptureMode) {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: skipped][DATA: reason=not_in_capture_mode]`);
        return false;
      }

      const cleaned = normalizeText(text);
      if (!cleaned) {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: skipped][DATA: reason=empty_transcript]`);
        return true;
      }

      console.log(`[FLOW: sales][STEP: email_capture][STATUS: processing][DATA: attempt=${emailCaptureAttempt + 1} input="${cleaned}" bufferLen=${emailCaptureBuffer.length}]`);
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: debug][DATA: confirmPending=${emailCaptureConfirmPending} confirmAsked=${emailCaptureConfirmAsked}]`);

      // ── Phase 2: Waiting for YES/NO confirmation ────────────────
      if (emailCaptureConfirmPending && emailCaptureConfirmAsked) {
        console.log(`[FLOW: sales][STEP: email_confirmation][STATUS: awaiting][DATA: email="${emailCaptureConfirmPending}" input="${cleaned}"]`);
        TimerManager.clearEmailConfirm();
        ...
        if (isYes) {
          ...
          console.log(`[FLOW: sales][STEP: email_confirmation][STATUS: success][DATA: email="${confirmedEmail}" userSaid="yes"]`);
          ...
          console.log(`[FLOW: sales][STEP: email_confirmed][STATUS: locked][DATA: email="${confirmedEmail}" is_confirmed=true createTicketBlocked=false]`);
          ...
          if (openaiWs?.readyState === WebSocket.OPEN) {
            openaiWs.send(JSON.stringify({
              type: "conversation.item.create",
              item: { type: "message", role: "user", content: [{ type: "input_text", text: userMsg }] },
            }));
            ...
            console.log(`[FLOW: sales][STEP: create_ticket_trigger][STATUS: pending][DATA: email="${confirmedEmail}" allReady=true]`);
            openaiWs.send(JSON.stringify({
              type: "conversation.item.create",
        ...
        if (isNo) {
          console.log(`[FLOW: sales][STEP: email_confirmation][STATUS: rejected][DATA: email="${emailCaptureConfirmPending}" userSaid="no"]`);
          emailCaptureAttempt++;
          emailCaptureConfirmPending = null;
          emailCaptureConfirmAsked = false;
          ...
          console.log(`[FLOW: sales][STEP: email_capture][STATUS: retry][DATA: attempt=${emailCaptureAttempt}/${EMAIL_MAX_ATTEMPTS} bufferCleared=true confirmed=false]`);

          if (emailCaptureAttempt >= EMAIL_MAX_ATTEMPTS) {
            console.warn(`[FLOW: sales][STEP: email_capture][STATUS: max_retries][DATA: attempts=${EMAIL_MAX_ATTEMPTS}]`);
            createTicketBlockedForEmail = false;
            resetEmailCapture();
            if (openaiWs?.readyState === WebSocket.OPEN) {
              openaiWs.send(JSON.stringify({
                type: "conversation.item.create",
                item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: Email capture has failed after ${EMAIL_MAX_ATTEMPTS} attempts. Tell the customer you're having trouble capturing the email by voice and ask them to call 1300 101 414 or email support@infinetbroadband.com.au to complete their order. Be apologetic and warm.` }] },
              }));
              scheduleResponseCreate();
            }
            return true;
        ...
        console.log(`[FLOW: sales][STEP: email_confirmation][STATUS: ambiguous][DATA: input="${cleaned}" treating_as=re_spell]`);
        emailCaptureConfirmPending = null;
        emailCaptureConfirmAsked = false;
        transitionFSM(FSM_STATE.EMAIL_CAPTURE);
      ...
      if (looksLikeDomainCorrection && emailCaptureBuffer.length > 0) {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: domain_correction][DATA: bufferCleared=true]`);
      ...
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: parsing][DATA: combined="${combinedTranscript}" bufferLen=${emailCaptureBuffer.length}]`);
      const parsed = parseVoiceEmail(combinedTranscript);
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: parse_result][DATA: parsed="${parsed}" raw="${combinedTranscript}"]`);

      if (!parsed) {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: parse_failed][DATA: combined="${combinedTranscript}"]`);
        if (combinedTranscript.split(/\s+/).length < 3) {
          console.log(`[FLOW: sales][STEP: email_capture][STATUS: waiting][DATA: reason=buffer_too_short]`);
          return true;
        }
        if (openaiWs?.readyState === WebSocket.OPEN) {
          openaiWs.send(JSON.stringify({
            type: "conversation.item.create",
            item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: The customer is spelling their email but I couldn't parse it fully. Heard: "${combinedTranscript}". Ask them to repeat from the beginning, one letter at a time. For @ use 'at', for . use 'dot'. Be warm and patient.` }] },
          }));
          scheduleResponseCreate();
        }
      ...
      console.log(`[FLOW: sales][STEP: email_capture][STATUS: parsed_ok][DATA: email="${parsed}" requesting_confirmation=true]`);
    ...
    function initSalesStepMachine() {
      ...
      if (c.leadInterest && c._websiteCheckDone) {
        ...
        console.log(`[FLOW: sales][STEP: init_step_machine][STATUS: ok][DATA: startStep="${salesStep}" websiteCheckDone=true]`);
      } else {
        console.log(`[FLOW: sales][STEP: init_step_machine][STATUS: blocked][DATA: leadInterest=${!!c.leadInterest} websiteCheckDone=${!!c._websiteCheckDone}]`);
      }
    }
    ...
    function advanceSalesStep(completedStep) {
      ...
      if (next === "email" && c.email && email_state.is_confirmed) { advanceSalesStep("email"); return; }
      if (next === "createTicket" && c._firstName && c._lastName && c.phone && c.email && email_state.is_confirmed) {
        salesStep = "createTicket";
      } else {
        salesStep = next;
      }
      console.log(`[FLOW: sales][STEP: advance_step][STATUS: ok][DATA: from="${completedStep}" to="${salesStep}"]`);
    }
    ...
    function buildSalesStepHint() {
      ...
      if (c.leadInterest && c._websiteCheckRequired && !c._websiteCheckDone) {
        if (!c._websiteCheckAsked) {
          console.log(`[FLOW: sales][STEP: website_check][STATUS: not_asked][DATA: websiteCheckRequired=true websiteCheckDone=false]`);
          ...
          console.log(`[FLOW: sales][STEP: website_check][STATUS: asked_awaiting_answer][DATA: websiteCheckAsked=true websiteCheckDone=false]`);
      ...
      switch (salesStep) {
        ...
        case "email":
          ...
          if (email_state.value && email_state.is_confirmed) {
            console.log(`[FLOW: sales][STEP: email][STATUS: already_confirmed][DATA: email="${email_state.value}" skipping=true]`);
        ...
        case "createTicket": {
          ...
          if (createTicketBlockedForEmail) {
            console.log(`[FLOW: sales][STEP: create_ticket][STATUS: blocked][DATA: reason=email_not_confirmed is_confirmed=${email_state.is_confirmed}]`);
          ...
          if (missing.length > 0) {
            console.log(`[FLOW: sales][STEP: create_ticket][STATUS: missing_fields][DATA: missing=${JSON.stringify(missing)}]`);
            if (!c.phone) salesStep = "phone";
            else if (!c.email) salesStep = "email";
            return buildSalesStepHint();
          }

          console.log(`[FLOW: sales][STEP: create_ticket][STATUS: ready][DATA: name="${c._firstName} ${c._lastName}" phone="${c.phone}" email="${c.email}" plan="${c.leadInterest}"]`);
...
YOU MUST CALL create_ticket IMMEDIATELY. Do NOT say anything to the user first. CALL THE TOOL.`;
    ...
    function scheduleResponseCreate(contextHint = null, delayMs = 0, force = false) {
      ...
      const send = () => {
        ...
        if (combinedHint) {
          openaiWs.send(JSON.stringify({
            type: "conversation.item.create",
            item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: ${combinedHint}` }] },
          }));
        }
    ...
    function detectPhoneVerificationRequest(text) {
      ...
      return lower.includes("phone") || lower.includes("contact number") || lower.includes("mobile number") || lower.includes("number on the account");
    }
    ...
    function wasLastMessageNetworkQuestion() {
      ...
      for (let i = msgs.length - 1; i >= 0; i--) {
        if (msgs[i].role === "assistant") {
          ...
          return (t.includes("nbn") && t.includes("opticomm")) ||
    ...
    function detectPlanPresentation(text) {
      if (!text) return false;
      const lower = text.toLowerCase();
      return (lower.includes("mbps") && (lower.includes("$") || lower.includes("per month") || lower.includes("/m"))) ||
    ...
    function detectWebsiteCheckQuestion(text) {
      if (!text) return false;
      const lower = text.toLowerCase();
      return lower.includes("check out our website") || lower.includes("visited our website") ||
        lower.includes("had a chance to check out") || lower.includes("seen the plans or pricing") ||
    ...
    function detectEmailSpellingRequest(text) {
      if (!text) return false;
      const lower = text.toLowerCase();
      return (lower.includes("spell") && lower.includes("email")) ||
    ...
    function detectSalesStepAnswer(text) {
      ...
      if (salesStep === "firstName") {
        ...
        if (firstName && firstName.length > 1) {
          ...
          sessions.set(session.id, session);
          console.log(`[FLOW: sales][STEP: firstName][STATUS: captured][DATA: firstName="${firstName}"]`);
        ...
        if (lastName && lastName.length > 1) {
          ...
          sessions.set(session.id, session);
          console.log(`[FLOW: sales][STEP: lastName][STATUS: captured][DATA: lastName="${lastName}" fullName="${session.collected.name}"]`);
        ...
        if (digits.length >= 8) {
          ...
          sessions.set(session.id, session);
          console.log(`[FLOW: sales][STEP: phone][STATUS: captured][DATA: phone="${digits}"]`);
    ...
    function openElevenLabsStream(force = false) {
      if (!force && elevenLabsWs && (elevenLabsWs.readyState === WebSocket.OPEN || elevenLabsWs.readyState === WebSocket.CONNECTING)) {
        return;
      }
      closeElevenLabsWs();
      ...
      elWs.on("message", (data) => {
        try {
          ...
          if (isFinal) {
            ...
            console.log(`🔊 [EL] TTS generation complete (isFinal) — waiting for client audio_done`);
    ...
    function interruptElevenLabsStream() {
      ...
      try { elevenLabsWs.send(JSON.stringify({ text: "" })); } catch (e) { console.warn("[EL] interrupt flush failed:", e.message); }
      try {
        elevenLabsWs.send(JSON.stringify({ text: " ", voice_settings: { stability: 0.4, similarity_boost: 0.75, speed: 1.1 }, xi_api_key: ELEVENLABS_API_KEY }));
        elevenLabsReady = true;
        elevenLabsStreaming = true;
      } catch (e) {
    ...
    function closeElevenLabsWs() {
      if (elevenLabsWs) {
        ...
        try {
          ...
          else if (elevenLabsWs.readyState === WebSocket.OPEN) elevenLabsWs.close();
        } catch (err) {
    ...
    function handleOpenAIEvent(event) {
      ...
      switch (event.type) {
        ...
        case "conversation.item.input_audio_transcription.completed": {
          ...
          console.log(`📊 [TRANSCRIPT] raw="${event.transcript}" cleaned="${cleaned}"`);
          ...
          // ─────────────────────────────────────────────────────────
          const isEmailConfirmResponse = emailCaptureMode && emailCaptureConfirmAsked;

          if (assistantSpeaking && !(looksLikeEmail || looksLikePhone) && !isEmailConfirmResponse) {
            console.log(`🔇 Ignoring transcript during assistant speech (not email confirm)`);
            break;
          }

          if (isEmailConfirmResponse) {
            console.log(`[FLOW: sales][STEP: email_confirmation][STATUS: processing][DATA: input="${cleaned}" allowedDuringAssistantSpeech=true]`);
          }
          ...
          if (awaitingPhoneVerification && looksLikePhone) {
            ...
            if (digits.length >= 6) {
              ...
              console.log(`[FLOW: support][STEP: phone_verification][STATUS: buffered][DATA: phone="${rawPhoneBuffer}"]`);
            }
          }

          console.log(`[FLOW: ${session?.collected?.intent || 'unknown'}][STEP: transcript][STATUS: ok][DATA: emailMode=${emailCaptureMode} salesStep=${salesStep} looksEmail=${looksLikeEmail} looksSpelling=${looksLikeVoiceEmailSpelling(cleaned)}]`);
          ...
          if (mappedNetwork && wasLastMessageNetworkQuestion()) {
            ...
            if (openaiWs?.readyState === WebSocket.OPEN) {
              openaiWs.send(JSON.stringify({
                type: "conversation.item.create",
                item: { type: "message", role: "user", content: [{ type: "input_text", text: clarified }] },
              }));
              scheduleResponseCreate();
            }
            TimerManager.resetSilence();
          ...
          if (session.collected._websiteCheckRequired && !session.collected._websiteCheckDone && detectWebsiteCheckAnswer(cleaned)) {
            ...
            if (lastAiMsg && detectWebsiteCheckQuestion(lastAiMsg.content || "")) {
              ...
              sessions.set(session.id, session);
              console.log(`[FLOW: sales][STEP: website_check][STATUS: answered][DATA: answer="${cleaned}" websiteCheckDone=true]`);
              initSalesStepMachine();
            }
          }
        ...
        case "response.created":
          ...
          // Clear watchdog — response is now active
          TimerManager.clearWatchdog();
        ...
        case "response.text.done":
          if (event.text) {
            ...
            if (detectPlanPresentation(event.text)) {
              ...
              console.log(`[FLOW: sales][STEP: plan_presentation][STATUS: detected][DATA: isPackage=true timer=20s]`);
            }
            ...
            if (
              ...
              console.log(`[FLOW: sales][STEP: email_capture][STATUS: activating][DATA: reason=ai_asked_for_email_spelling]`);
              startEmailCapture();
            }

            // FIX (Issue 2): Track website check question asked
            if (session.collected.leadInterest && session.collected._websiteCheckRequired && !session.collected._websiteCheckAsked && detectWebsiteCheckQuestion(event.text)) {
              session.collected._websiteCheckAsked = true;
              sessions.set(session.id, session);
              console.log(`[FLOW: sales][STEP: website_check][STATUS: asked][DATA: websiteCheckAsked=true]`);
            }
          }
          break;
        ...
        case "response.done": {
          ...
          const hasTextOutput = outputItems.some((item) =>
            item.type === "message" && item.content?.some((c) => c.type === "text" && c.text?.trim())
          ...
          if (!hasFunctionCall && !hasTextOutput && pendingFunctionCalls === 0 && !finalMessageLock) {
            ...
            emptyResponseCount++;
            console.warn(`⚠️ EMPTY RESPONSE: attempt ${emptyResponseCount}/${MAX_EMPTY_RETRIES}`);
          ...
          if (pendingPostDoneCreate && pendingFunctionCalls === 0) {
            ...
            if (createTicketBlockedForEmail) {
              console.log(`[FLOW: sales][STEP: response_done][STATUS: post_done_blocked][DATA: reason=email_not_confirmed createTicketBlockedForEmail=true]`);
              pendingPostDoneCreate = false;
          ...
          if (!pendingFunctionCalls) {
            ...
            if (fsmState !== FSM_STATE.EMAIL_CAPTURE && fsmState !== FSM_STATE.EMAIL_CONFIRMATION) {
              transitionFSM(FSM_STATE.LISTENING);
            }
            socket.emit("status", "listening");
          }
    ...
    async function handleFunctionCall(item) {
      ...
      if (fn === "verify_phone" && !session.collected._emailVerifiedCustomerId) {
        console.log(`[FLOW: sales][STEP: verify_phone][STATUS: blocked][DATA: reason=sales_flow_no_customer_id]`);
        ...
        if (phoneToSave) {
          ...
          console.log(`[FLOW: sales][STEP: phone][STATUS: saved][DATA: phone="${session.collected.phone}"]`);
        }
        const fakeResult = JSON.stringify({ success: true, _redirected: true, message: "Phone number saved." });
        if (openaiWs?.readyState === WebSocket.OPEN) {
          openaiWs.send(JSON.stringify({ type: "conversation.item.create", item: { type: "function_call_output", call_id, output: fakeResult } }));
        }
        ...
        if (pendingFunctionCalls === 0 && openaiWs?.readyState === WebSocket.OPEN) {
          ...
          openaiWs.send(JSON.stringify({ type: "conversation.item.create", item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: ${hint}` }] } }));
          if (fsmState !== FSM_STATE.EMAIL_CAPTURE && fsmState !== FSM_STATE.EMAIL_CONFIRMATION) {
            transitionFSM(FSM_STATE.LISTENING);
          }
          scheduleResponseCreate();
      ...
      if (fn === "verify_phone" && rawPhoneBuffer) {
        console.log(`[FLOW: support][STEP: verify_phone][STATUS: override][DATA: llmPhone="${args.phone}" bufferPhone="${rawPhoneBuffer}"]`);
      ...
      let systemHint = `[FLOW: ${session.collected?.intent || 'unknown'}] Current collected fields: ${JSON.stringify(
        Object.fromEntries(Object.entries(session.collected || {}).filter(([k]) => k !== "_registeredPhone" && k !== "_rp"))
      ...
      if (fn === "customer_lookup") {
        ...
        if (parsedResult?._blocked && parsedResult?.reason === "sales_flow") {
          console.log(`[FLOW: sales][STEP: customer_lookup][STATUS: blocked_hint][DATA: reason=sales_flow]`);
      ...
      if (fn === "create_ticket") {
        ...
        if (parsedResult?._blocked && parsedResult?.reason === "email_missing") {
          ...
          console.log(`[FLOW: sales][STEP: create_ticket][STATUS: blocked][DATA: reason=email_not_confirmed salesStep_forced_to=email captureStarted=true]`);
          ...
          console.log(`[FLOW: sales][STEP: create_ticket][STATUS: success][DATA: isSales=${isSales} ticketId=${ticketId}]`);
          if (isSales) {
          ...
          TimerManager.releaseFinalLock();
          console.log(`[FLOW: sales][STEP: create_ticket][STATUS: failed][DATA: error="${parsedResult?.error}"]`);
      ...
      if (fn === "extract_call_fields") {
        ...
        const shouldGate = c.leadInterest && c._websiteCheckRequired && !c._websiteCheckAsked && !c._websiteCheckDone;
        if (shouldGate) {
          console.log(`[FLOW: sales][STEP: website_check][STATUS: gating][DATA: leadInterest="${c.leadInterest}" websiteCheckDone=false]`);
      ...
      if (pendingFunctionCalls === 0 && openaiWs?.readyState === WebSocket.OPEN) {
        openaiWs.send(JSON.stringify({
          type: "conversation.item.create",
          item: { type: "message", role: "user", content: [{ type: "input_text", text: `[SYSTEM_CONTEXT]: ${systemHint}\n\nIMPORTANT: Respond immediately based on the tool result above.` }] },
        }));

        if (fsmState === FSM_STATE.TOOL_EXECUTING && fsmState !== FSM_STATE.EMAIL_CAPTURE && fsmState !== FSM_STATE.EMAIL_CONFIRMATION && fsmState !== FSM_STATE.FINAL) {
          transitionFSM(FSM_STATE.LISTENING);
        }
    ...
    async function execTool(fn, args) {
      if (fn === "extract_call_fields") {
        ...
        if (args.email) {
          ...
          sessions.set(session.id, session);
          console.log(`[FLOW: sales][STEP: email_capture][STATUS: extracted_by_llm][DATA: email="${args.email}"]`);
          if (salesStep === "email") advanceSalesStep("email");
        }
      ...
      if (fn === "customer_lookup") {
        ...
        // ─────────────────────────────────────────────────────────
        const isSalesFlow = !!(session.collected?.leadInterest) && !(session.collected?._emailVerifiedCustomerId);
        if (isSalesFlow) {
          console.log(`[FLOW: sales][STEP: customer_lookup][STATUS: blocked][DATA: reason=sales_flow_new_lead leadInterest="${session.collected.leadInterest}"]`);
          return JSON.stringify({
            success: false,
            _blocked: true,
            reason: "sales_flow",
            message: "New sales lead — customer lookup not performed. Treat as new customer. Collect name, phone, email, then call create_ticket."
          });
        }
        ...
        console.log(`[FLOW: support][STEP: customer_lookup][STATUS: executing][DATA: email="${lookupArgs.email}"]`);
        try {
          ...
          if (result.success && result.customer) {
            ...
            sessions.set(session.id, session);
            console.log(`[FLOW: support][STEP: customer_lookup][STATUS: found][DATA: customerId="${result.customer.id}"]`);
          ...
          email_state.is_confirmed = false;
          sessions.set(session.id, session);
          console.log(`[FLOW: support][STEP: customer_lookup][STATUS: not_found][DATA: email="${lookupArgs.email}"]`);
          return JSON.stringify({ ...result, _emailCleared: true, message: "No account found with that email. Please check and try again." });
        } catch (e) {
      ...
      if (fn === "verify_phone") {
        ...
        if (normalizedInput !== normalizedRegistered) {
          console.log(`[FLOW: support][STEP: verify_phone][STATUS: failed][DATA: mismatch=true]`);
          return JSON.stringify({ success: false, verificationFailed: true, message: "Phone number does not match the registered number." });
        }
        session.collected._phoneVerified = true;
        sessions.set(session.id, session);
        console.log(`[FLOW: support][STEP: verify_phone][STATUS: passed][DATA: customerId="${emailCustomerId}"]`);
      ...
      if (fn === "create_ticket") {
        ...
        if (!isSupportTicket && (!collected.email || !email_state.is_confirmed)) {
          console.warn(`[FLOW: sales][STEP: create_ticket][STATUS: blocked][DATA: email="${collected.email}" is_confirmed=${email_state.is_confirmed} reason=email_not_confirmed]`);
          ...
          return JSON.stringify({
            ...
            message: "SALES STEP [email]: Ask for email by voice spelling mode. Do NOT retry create_ticket until email_state.is_confirmed=true.",
          });
        }
        ...
        try {
          if (isSupportTicket) {
            console.log(`[FLOW: support][STEP: create_ticket][STATUS: creating][DATA: subject="${fa.subject}" customerId="${fa.customer_id}"]`);
            const r = await splynx.request("POST", "admin/support/tickets", objectToUrlEncoded(fa));
            console.log(`[FLOW: support][STEP: create_ticket][STATUS: success][DATA: ticketId="${r.id}"]`);
            const emailResult = await sendTicketEmail(r.id, fa, collected, true);
            ticketResult = { success: true, ticket_id: r.id, email_sent: emailResult.sent, email_error: emailResult.reason || null, _isSalesTicket: false, _ticketCompleted: true };
          } else {
            console.log(`[FLOW: sales][STEP: create_ticket][STATUS: sending_email][DATA: subject="${fa.subject}"]`);
            const emailResult = await sendTicketEmail(null, fa, collected, false);
            ticketResult = { success: true, message: "Sales inquiry submitted successfully", email_sent: emailResult.sent, email_error: emailResult.reason || null, _isSalesTicket: true, _ticketCompleted: true };
          }
        } catch (err) {
          console.error(`[FLOW: unknown][STEP: create_ticket][STATUS: error][DATA: error="${err.message}"]`);
    ...
    socket.on("audio_chunk", (b64) => {
      const shouldSuppress = awaitingStructuredInput || pendingFunctionCalls > 0 || session.finalLock || finalMessageLock;
    ...
    socket.on("audio_done", () => {
      ...
      if (fsmState !== FSM_STATE.EMAIL_CAPTURE && fsmState !== FSM_STATE.EMAIL_CONFIRMATION && fsmState !== FSM_STATE.FINAL) {
        transitionFSM(FSM_STATE.LISTENING);
      }
      ...
      lastResponseWasPackage = false;
      console.log(`⏱️  [TMgr] TTS finished → silence timer starting (${isPackage ? "20s package" : "15s normal"})`);
    ...
    socket.on("structured_input", (payload) => {
      ...
      if (field === "email") {
        console.log(`[FLOW: sales][STEP: email_capture][STATUS: structured_input][DATA: email="${value}"]`);
        ...
        if (openaiWs?.readyState === WebSocket.OPEN) {
          openaiWs.send(JSON.stringify({ type: "conversation.item.create", item: { type: "message", role: "user", content: [{ type: "input_text", text: userMessage }] } }));
          const salesHint = buildSalesStepHint() || "";
      ...
      console.log(`[FLOW: unknown][STEP: structured_input][STATUS: ok][DATA: field="${field}" value="${value}"]`);
      TimerManager.clearSilence();
      awaitingStructuredInput = false;
      structuredInputField = null;
    ...
    socket.on("disconnect", () => {
      ...
      console.log(`[FLOW: ${session?.collected?.intent || 'unknown'}][STEP: disconnect][STATUS: cleanup][DATA: email="${email_state.value}" confirmed=${email_state.is_confirmed} collected=${JSON.stringify(session?.collected || {})}]`);
    ...
    (async () => {
      try {
        ...
        console.log(`[FLOW: init][STEP: connect][STATUS: pending][DATA: sessionId="${session.id}"]`);
        await connectOpenAI();
        ...
        if (!session.hasGreeted) {
          ...
          console.log(`[FLOW: init][STEP: greeting][STATUS: sending][DATA: sessionId="${session.id}"]`);
          if (openaiWs?.readyState === WebSocket.OPEN) {
            openaiWs.send(JSON.stringify({ type: "response.create" }));
          } else {
            console.warn(`⚠️ OpenAI WS not open for greeting (state: ${openaiWs?.readyState})`);
          }
          sessions.set(session.id, session);
        } else {
          transitionFSM(FSM_STATE.LISTENING);
