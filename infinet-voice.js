...
// - BEFORE calling create_ticket, you MUST summarise the collected details back to the customer and ask: "I've got all your details — shall I go ahead and submit this for you?"
// - WAIT for the customer to explicitly say YES before calling create_ticket.
// - If they say NO or want to change something, ask what they'd like to update.
...
// - After create_ticket success for EXISTING customers: "Brilliant, all done! I've raised a support ticket for you and you'll get all the details sent through to your email shortly."
// - After create_ticket success for NEW customers (sales): "Awesome, you're all set! I've submitted your enquiry and our sales team will be reaching out to you via email shortly."
// - IMPORTANT: For sales inquiries (new customers), do NOT mention any ticket number or ticket ID.
// - For support: collect issueSummary with follow-up details.
// - Use customer_lookup for existing customers.
...
//   if (funcName === "create_ticket") {
//     // ── TICKET CONFIRMATION GATE for chat API ──
//     // If user hasn't confirmed yet, block the call
//     if (!session.collected._ticketConfirmed) {
//       session.collected._ticketPendingConfirmation = true;
//       session.collected._pendingTicketArgs = { ...args };
//       sessions.set(session.id, session);
//       return JSON.stringify({
//         success: false,
//         _blocked: true,
...
//         message:
//           "Before creating the ticket, you must ask the customer: 'I've got all your details — shall I go ahead and submit this for you?' Wait for their explicit YES before proceeding.",
//       });
//     }
...
//       detailLines.push(`Selected Plan: ${collected.leadInterest}`);
//     if (collected.paymentDate)
//       detailLines.push(`Payment extension until: ${collected.paymentDate}`);
...
//     delete session.collected._ticketPendingConfirmation;
//     delete session.collected._pendingTicketArgs;
...
//     if (collected.customer_id)
//       detailLines.push(`Customer ID: ${collected.customer_id}`);
//     detailLines.push("Issue: Customer unable to login to portal");
//     const detailsBlock = `\n\n--- Customer Details ---\n${detailLines.join("\n")}`;
//     const messageBody = `${args.message || "Customer requested assistance with portal login"}${detailsBlock}`;
//     const emailArgs = {
//       subject: "Support - Portal Login Assistance",
...
//     // ── Handle ticket confirmation flow for chat API ──
//     let parsedToolContent = safeParseJSON(toolContent);
//     if (
//       fn === "create_ticket" &&
//       parsedToolContent?._blocked &&
//       parsedToolContent?.reason === "confirmation_required"
//     ) {
//       // The tool was blocked — LLM needs to ask for confirmation
//       const contextHint = `TICKET CONFIRMATION REQUIRED: Summarise the customer's details and ask "I've got all your details — shall I go ahead and submit this for you?" Wait for their YES before calling create_ticket again. Current collected fields: ${JSON.stringify(session.collected || {})}.`;
//       const finalMessages = [
//         { role: "system", content: SYSTEM_PROMPT },
//         ...session.messages,
//         { role: "system", content: contextHint },
...
//       const text =
//         finalResp.choices?.[0]?.message?.content?.trim() ||
//         "Shall I go ahead and submit this for you?";
//       session.messages.push({ role: "assistant", content: text });
//       return text;
//     }
...
//     return text;
//   } else if (msg?.content) {
//     // ── Check if user is confirming ticket creation in chat flow ──
//     const userLastMsg = [...session.messages]
//       .reverse()
//       .find((m) => m.role === "user");
//     if (session.collected._ticketPendingConfirmation && userLastMsg) {
//       const lower = (userLastMsg.content || "").toLowerCase().trim();
//       const isYes =
//         /\b(yes|yeah|yep|yup|go ahead|please|sure|do it|submit|absolutely|correct|confirmed|confirm|sounds good|perfect|let's do it)\b/.test(
//           lower,
//         );
//       const isNo =
//         /\b(no|nope|don't|wait|hold on|cancel|stop|not yet|hang on|actually no|never mind)\b/.test(
//           lower,
//         );

//       if (isYes) {
//         // User confirmed — set flag and retry create_ticket
//         session.collected._ticketConfirmed = true;
//         sessions.set(session.id, session);

//         // Re-run create_ticket with the stored args
//         const storedArgs = session.collected._pendingTicketArgs || {};
//         session.messages.push({ role: "assistant", content: msg.content });

//         let toolContent;
//         try {
//           toolContent = await handleToolCall(
//             session,
//             "create_ticket",
//             storedArgs,
//           );
...
//         });
//         const text =
//           finalResp.choices?.[0]?.message?.content?.trim() || "All done!";
//         session.messages.push({ role: "assistant", content: text });
//         return text;
//       } else if (isNo) {
//         // User declined — clear pending state and ask what to change
//         delete session.collected._ticketPendingConfirmation;
//         delete session.collected._pendingTicketArgs;
//         sessions.set(session.id, session);

//         const declineText =
//           "No worries at all! Is there anything you'd like to change or update before I submit?";
//         session.messages.push({ role: "assistant", content: declineText });
//         return declineText;
//       }
//     }
