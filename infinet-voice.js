...
// ═══════════════════════════════════════════════════════════
//  [NEW] Scraper API URL — Python Flask service
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
//  [CHANGED] Updated SYSTEM_PROMPT with scraper-based flow
// ═══════════════════════════════════════════════════════════
const SYSTEM_PROMPT = `
You are a concise, professional voice/chat assistant for ${BRAND}.
Handle five call types / chat intents: support, sales, general, account, moving-relocating.
...
1. After getting the name (Preferred Name), immediately ask: "Are you a new or existing customer?"
   - When they answer, ALWAYS use extract_call_fields to store customerType: "new" or "existing".
2. If new → Proceed to SALES FLOW.
3. If existing → Ask what they need help with: support, accounts, or moving/relocating. Then proceed to the matching flow.

═══════════════════════════════════════════════
SALES FLOW (for new customers only):
═══════════════════════════════════════════════
1. Say: "Great! We offer high-speed NBN and OptiComm broadband plans for both residential and business customers. Is this for a residential or business connection?"
   → Save with extract_call_fields (residentialPreference).

2. Ask the network preference using EXACT wording: "First option is NBN. Second option is Opticomm. Which one would you prefer?"
   If user says "first", "1", "one", or "option 1" → NBN.
   If user says "second", "2", "two", or "option 2" → Opticomm.
   → Save with extract_call_fields (networkPreference).

3. Ask: "Could you please provide the full address where you need the connection?"
   (An input box will appear for the user to type their address accurately.)
   → Save address with extract_call_fields.

4. Once you have address + plan_type + network preference, say: "Please wait a moment while I check available plans at your address."
   Then IMMEDIATELY call scrape_address_plans with:
   - address: the user's address
   - plan_type: "Residential" or "Business" based on residentialPreference
   - network: "NBN" or "Opticomm" based on networkPreference

5. After scrape_address_plans returns → Display packages as a NUMBERED list:
   "Here are the plans available at your address:
   1. [Plan Name] — [Price]/month | [Download Speed] down / [Upload Speed] up [Promo if any]
   2. ...
   Which plan would you like? Just say the number."

6. After user selects a plan (by number), save leadInterest with extract_call_fields.

7. Ask: "Could you please provide your email address?"
   (An input box will appear for accurate email entry.)
   → Save email with extract_call_fields.

8. Say: "Please wait a moment while I process your request."
   Then call create_ticket with:
   - subject: "Sales Inquiry — [selected plan name]"
   - message: { message: "New customer inquiry from [preferredName]:\\nPlan: [leadInterest]\\nAddress: [address]\\nType: [residentialPreference]\\nNetwork: [networkPreference]\\nEmail: [email]" }
   - priority: "medium"
   - reporter_type: "api"

9. After success: "Thank you \${preferredName}! I have raised a sales inquiry ticket for you. You will receive the details via email shortly. Our team will contact you shortly."

═══════════════════════════════════════════════
SUPPORT FLOW (for existing customers only):
═══════════════════════════════════════════════
- Ask: "Could you please provide your email address so I can look up your account?" (Input box appears for email.)
- Call customer_lookup with the email.
- After successful lookup: Reply EXACTLY: "Found your account successfully, \${preferredName}. Please provide a brief description of the issue you're facing."
- Do NOT list or mention any active services automatically (this is not for support flow).
- If issueSummary is not collected, ask: "Please provide a brief description of the issue." Once a basic description is provided, immediately ask for additional high-level details to help our support team (e.g. "Any more details like when it started, affected devices, error messages, or speed test results?").
- Combine everything into the final issueSummary.
- When ALL details collected (preferredName, customer_id, email, issueSummary) → Say "Please wait a moment while I process your request." → Call create_ticket with customer_id, subject based on issueSummary, message: full issueSummary, reporter_type: 'api', priority: 'medium', type_id for support.

═══════════════════════════════════════════════
ACCOUNTS FLOW (billing/financing, for existing customers only):
═══════════════════════════════════════════════
- Ask for email (input box) → call customer_lookup to get customer_id and services.
- If not found, say "Sorry, I couldn't find your account. Are you sure you're an existing customer?" and switch to sales if needed.
- Answer any billing or payment questions using the Knowledge base.
- Specifically for "pay a bill" or any question about paying over the phone: Reply concisely: "We can take payments or update payment details over the phone. Please call 1300 101 414 to proceed. Would you like help with anything else regarding your bill?"
- For any specific issue → collect issueSummary.
- When ALL details collected → Say "Please wait a moment while I process your request." → Call create_ticket with customer_id, subject: "Accounts Query: [brief summary]", message: issueSummary, reporter_type: 'api', priority: 'medium', type_id for accounts.

═══════════════════════════════════════════════
RELOCATION FLOW (existing customers, moving/relocating):
═══════════════════════════════════════════════
- Ask for email (input box) → customer_lookup (lists active services).
- Ask which service to terminate and termination date.
- Ask residential/business preference for new address.
- Ask NBN/Opticomm preference.
- Ask for new address (input box).
- Call scrape_address_plans for new address with plan_type and network.
- Show available plans → user selects by number.
- Ask for desired connection date.
- When ALL details collected (preferredName, customer_id, email, leadInterest, address (new), terminationDate, connectionDate, serviceToTerminate, residentialPreference, networkPreference) → Call create_ticket with:
    - customer_id (looked-up ID)
    - subject: "Relocation Request — [leadInterest]"
    - message: { message: "Relocation from \${preferredName}:\\nOld service to terminate: \${serviceToTerminate} on \${terminationDate}\\nNew address: \${address}\\nNew connection date: \${connectionDate}\\nNew plan: \${leadInterest}\\nResidential/Business: \${residentialPreference}\\nNetwork: \${networkPreference}\\nFull conversation summary." }
    - priority: "medium"
    - reporter_type: "api"
    - lead_id: 0 (but customer_id present)
    After success: "Thank you \${preferredName}! I have raised a sales inquiry ticket for you. You will receive the details via email shortly. Our team will contact you shortly."

GENERAL: Answer using the Knowledge base. If needed, ask clarifying questions concisely. When the customer asks about plans, pricing, speeds, upgrades or "what plans do you have?": Follow the STRICT PLANS DISPLAY RULE (ask residential/business first, then NBN/OptiComm), then call scrape_address_plans or get_internet_plans tool. After result, show ONLY matching plans.

═══════════════════════════════════════════════
TOOL USAGE (CRITICAL):
═══════════════════════════════════════════════
- When the customer answers the "new or existing" question, ALWAYS use extract_call_fields to capture customerType: "new" or "existing".
- For new customers → sales@ email will be used. For existing customers → support@ email will be used.
- For plans at a specific address: ALWAYS use scrape_address_plans (NOT get_internet_plans). The scraper takes 10-30 seconds so ALWAYS tell the user "Please wait a moment while I check available plans at your address." BEFORE calling it.
- get_internet_plans is ONLY a fallback if scraper is unavailable.
- To lookup customer by name, email, or phone: call customer_lookup with at least one of name, email, phone. Returns customer details and ACTIVE services only.
- The tool results will be injected into the conversation. ALWAYS use the live tool data for plans and availability (never rely on old hardcoded KB plans).
- After a tool result, continue the flow concisely using the live data.
- Call extract_call_fields ONLY when identification info is provided (like Name, Email, Address, or Phone) or right before creating a ticket. Do NOT call it for intermediate choices like residential/business or NBN/OptiComm to ensure the conversation stays fast and "snappy".
- When asking for the network preference, ALWAYS use this exact wording: "First option is NBN. Second option is Opticomm. Which one would you prefer?" If the user says "first", "1", "one", or "option 1", you MUST select NBN. If they say "second", "2", "two", or "option 2", you MUST select Opticomm.
...
const extractFunction = {
  name: "extract_call_fields",
  description: "Extract fields from user message: intent (support/sales/general/account), issueSummary, preferredName (what they want to be called), email, priority, callbackRequest (boolean), timeline, leadInterest, accountNumber, name, phone, terminationDate, connectionDate, serviceToTerminate, residentialPreference, networkPreference, address. Omit fields not present.",
  parameters: {
    type: "object",
    properties: {
      intent: { type: "string", enum: ["support", "sales", "general", "account"] },
...
const getPlansTool = {
  name: "get_internet_plans",
  description: "Fetch the latest live internet tariff plans from Splynx (fallback). Prefer scrape_address_plans when the customer's address is known.",
  parameters: { type: "object", properties: {}, required: [] },
};
...
// ═══════════════════════════════════════════════════════════
//  [NEW] Scraper tool definition
// ═══════════════════════════════════════════════════════════
const scrapeAddressPlansTool = {
  name: "scrape_address_plans",
  description: "Scrape live broadband plans available at a specific address from the InfiNET website. Use this INSTEAD of get_internet_plans when you have the customer's address, residential/business preference, and network preference (NBN/Opticomm). Returns real-time pricing and plan data. The scraper takes 10-30 seconds so tell the user 'Please wait a moment while I check available plans at your address.' BEFORE calling this.",
  parameters: {
    type: "object",
    properties: {
      address: { type: "string", description: "Full street address including suburb, state and postcode" },
      plan_type: { type: "string", enum: ["Residential", "Business"], description: "Residential or Business plans" },
      network: { type: "string", enum: ["NBN", "Opticomm", ""], description: "Filter by network. Empty string = show all." },
    },
    required: ["address"],
  },
};
...
// ═══════════════════════════════════════════════════════════
//  [CHANGED] Added scrapeAddressPlansTool to tools array
// ═══════════════════════════════════════════════════════════
const tools = [
  extractFunction,
  getPlansTool,
  checkAvailabilityTool,
  scrapeAddressPlansTool,
  customerLookupTool,
  createTicketTool,
  getTicketTypesTool,
  getTicketGroupsTool,
...
// ═══════════════════════════════════════════════════════════
//  [NEW] Helper to call the Python scraper API
// ═══════════════════════════════════════════════════════════
async function callScraperApi(address, planType, network) {
  ...
  try {
    ...
    if (firstMsg?.function_call) {
      ...
      if (funcName === "extract_call_fields") {
        ...
        // [CHANGED] Try scraper first if address is available, fallback to Splynx
        ...
        if (addr) {
          try {
            const scraperResult = await callScraperApi(addr, planType, network);
            toolContent = JSON.stringify(scraperResult);
          } catch (err) {
            console.warn("Scraper failed for get_internet_plans, falling back to Splynx:", err.message);
            const tariffs = await fetchTariffs();
            toolContent = JSON.stringify({
              success: true,
              plans: tariffs.map((t) => ({
        ...
        // ═══════════════════════════════════════════════════════════
        //  [NEW] scrape_address_plans handler
        // ═══════════════════════════════════════════════════════════
      } else if (funcName === "scrape_address_plans") {
        const { address, plan_type, network } = args;
        if (!address) {
          toolContent = JSON.stringify({ error: "Address is required" });
        } else {
          try {
            console.log(`🔍 Calling scraper API: ${address} (${plan_type || "Residential"}) network=${network || "all"}`);
            const scraperResult = await callScraperApi(address, plan_type, network);
            toolContent = JSON.stringify(scraperResult);
          } catch (err) {
            console.error("Scraper API failed:", err.message);
            toolContent = JSON.stringify({
              success: false,
              error: `Scraper unavailable: ${err.message}. Make sure scraper_api.py is running on ${SCRAPER_API_URL}`,
            });
          }
        }
        ...
        if (!address) {
          ...
          // [CHANGED] Try scraper first for real availability
          try {
            const planType = session.collected.residentialPreference === "business" ? "Business" : "Residential";
            const resp = await callScraperApi(address, planType, "");
            if (resp.success && resp.packages.length > 0) {
              const networks = [...new Set(resp.packages.map(p => p.network).filter(Boolean))];
              toolContent = JSON.stringify({
                success: true,
                address: address,
                available: true,
                networks: networks,
                totalPlans: resp.packages.length,
                message: `Address is serviceable. Available networks: ${networks.join(", ")}`,
              });
            } else {
              throw new Error("No plans found via scraper");
            }
          } catch (scraperErr) {
            console.warn("Scraper availability check failed, falling back:", scraperErr.message);
            const locId = await determineLocationId(address);
            const tariffs = await fetchTariffs();
            const availablePlans = locId
              ? tariffs.filter((t) => t.available_for_locations && t.available_for_locations.includes(locId))
            ...
            toolContent = JSON.stringify({
              ...
              locationId: locId,
              locationName: LOCATIONS.find((l) => l.id === locId)?.name || "Unknown",
              available: availablePlans.length > 0,
              availablePlans: availablePlans.map((p) => ({
                title: p.title,
                price: parseFloat(p.price),
                download: `${p.speed_download / 1000} Mbps`,
...
// ═══════════════════════════════════════════════════════════
//  [CHANGED] Structured Input — now supports "address" field
// ═══════════════════════════════════════════════════════════
app.post("/api/voice/structured-input", async (req, res) => {
  try {
    const { sessionId, type, field, value } = req.body || {};
    ...
    if (!["email", "phone", "address"].includes(field)) {
      return res.status(400).json({ error: "Field must be 'email', 'phone', or 'address'" });
    }
    const session = sessions.get(sessionId);
    if (!session) {
      return res.status(404).json({ error: "Session not found" });
    ...
    if (msg?.function_call) {
      ...
      if (funcName === "extract_call_fields") {
        ...
        // [NEW] Handle scrape_address_plans in structured input flow
      } else if (funcName === "scrape_address_plans") {
        const { address, plan_type, network } = args;
        if (!address) {
          toolContent = JSON.stringify({ error: "Address is required" });
          ...
          try {
            ...
            toolContent = JSON.stringify({ success: false, error: `Scraper unavailable: ${err.message}` });
          }
        }
        ...
        if (!address) {
          ...
          try {
            ...
            if (resp.success && resp.packages.length > 0) {
              const networks = [...new Set(resp.packages.map(p => p.network).filter(Boolean))];
              toolContent = JSON.stringify({ success: true, address, available: true, networks, totalPlans: resp.packages.length });
            } else {
            ...
            const locId = await determineLocationId(address);
            const tariffs = await fetchTariffs();
            const availablePlans = locId ? tariffs.filter(t => t.available_for_locations?.includes(locId)) : [];
            toolContent = JSON.stringify({
              success: true, address, locationId: locId,
              locationName: LOCATIONS.find(l => l.id === locId)?.name || "Unknown",
              available: availablePlans.length > 0,
              availablePlans: availablePlans.map(p => ({
                title: p.title, price: parseFloat(p.price),
                download: `${p.speed_download / 1000} Mbps`, upload: `${p.speed_upload / 1000} Mbps`,
              })),
            });
          }
        }
        ...
        if (addr) {
          try {
            toolContent = JSON.stringify(await callScraperApi(addr, planType, network));
          } catch (err) {
            const tariffs = await fetchTariffs();
            toolContent = JSON.stringify({
              success: true,
              plans: tariffs.map(t => ({
                id: t.id, title: t.title, price: parseFloat(t.price),
                download: `${t.speed_download / 1000} Mbps`, upload: `${t.speed_upload / 1000} Mbps`,
              })),
            });
          }
        } else {
...
httpServer.listen(PORT, () => {
  ...
  console.log(`🎤 Realtime API + ElevenLabs WebSocket • Ultra-low latency mode`);
  console.log(`🔌 Socket.IO ready for voice clients`);
  console.log(`🔍 Scraper API: ${SCRAPER_API_URL}`);
});