...
STEP 2: THEN call create_ticket IMMEDIATELY. Do NOT say anything to the user first. CALL THE TOOLS.`,
    ...
    //     // FIX: Clear streaming/speaking state from the prior response
    //     // (the AI was mid-speech saying "bear with me..." when this gate fired)
    //     // Without this, the next response.done hits the elevenLabsStreaming guard
    //     // and deadlocks — nothing retries, nothing plays, system goes silent.
    //     assistantSpeaking = false;
    //     safeSetElevenLabsStreaming(false);
    ...
    //         }),
    //       );
    //       // FIX: Use forceNow=true to bypass any stale isResponseActive state
    //       scheduleResponseCreate(null, 0, true);
    //     }
    //     return;
    //   }
    ...
    //     pendingTicketArgs = null;
    //     pendingTicketConfirmation = false;
    //     // FIX: Clear stale speaking state so the post-ticket response isn't blocked
    //     assistantSpeaking = false;
    //   }
    ...
    //       pendingTicketArgs = null;
    //       TimerManager.releaseFinalLock();
    //       // FIX: Clear streaming state so the success response isn't blocked
    //       // by stale elevenLabsStreaming=true from prior speech
    //       assistantSpeaking = false;
    //       safeSetElevenLabsStreaming(false);
    //       const ticketId = parsedResult.ticket_id;
    //       const isSales = parsedResult._isSalesTicket === true || !ticketId;
