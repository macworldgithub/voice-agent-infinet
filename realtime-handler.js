...
STEP 2: THEN call create_ticket IMMEDIATELY. Do NOT say anything to the user first. CALL THE TOOLS.`,
    ...
    // ═══════════════════════════════════════════════════════════════
    //  FORCE CREATE TICKET — directly executes ticket creation
    //  and feeds result back to LLM for the closing message
    // ═══════════════════════════════════════════════════════════════
    async function forceCreateTicket() {
      const c = session.collected || {};
      const fullNameForTicket =
        [c._firstName, c._lastName].filter(Boolean).join(" ") ||
        c.name ||
        c.preferredName ||
        "N/A";
      ...
      const ticketArgs = {
        ...
        leadInterest: c.leadInterest || undefined,
        subject: c.leadInterest
          ? `New Sales Enquiry - ${c.leadInterest}`
          : c.issueSummary
            ? `Support Request - ${c.issueSummary}`
            : "New Enquiry",
        message: {
          message:
            c.issueSummary || c.leadInterest
              ? `Customer enquiry: ${c.issueSummary || c.leadInterest}`
              : "Customer enquiry submitted via voice call.",
        },
      };

      // Merge args from a previous blocked attempt if available
      if (pendingTicketArgs?.args) {
        Object.assign(ticketArgs, pendingTicketArgs.args);
      }
      pendingTicketArgs = null;

      dbg("ticket", "forceCreateTicket", "executing", {
        name: fullNameForTicket,
        email: c.email,
        phone: c.phone,
        argsPreview: JSON.stringify(ticketArgs).substring(0, 150),
      });

      try {
        const ticketResult = await execTool("create_ticket", ticketArgs);
        dbg("ticket", "forceCreateTicket_RESULT", "done", {
          resultPreview: ticketResult.substring(0, 200),
        });

        let parsedTicketResult = null;
        try {
          parsedTicketResult = JSON.parse(ticketResult);
        } catch (_) {}

        let closingHint;
        if (parsedTicketResult?.success) {
          salesStep = "done";
          pendingTicketConfirmation = false;
          pendingTicketArgs = null;
          const ticketId = parsedTicketResult.ticket_id;
          const isSales =
            parsedTicketResult._isSalesTicket === true || !ticketId;
          if (isSales) {
            closingHint = `TICKET CREATED SUCCESSFULLY. Say to the customer: "Awesome, you're all set! Our sales team will be in touch via email shortly. Is there anything else I can help you with?"`;
          } else {
            closingHint = `TICKET #${ticketId} CREATED SUCCESSFULLY. Say to the customer: "Brilliant, all done! Ticket number ${ticketId} has been raised and the details have been sent to your email. Is there anything else I can help you with?"`;
          }
        } else {
          closingHint = `Ticket creation FAILED: ${parsedTicketResult?.error || "unknown error"}. Apologise to the customer and suggest calling 1300 101 414 for assistance.`;
        }

        // Feed the result into the conversation and force a response
        if (openaiWs?.readyState === WebSocket.OPEN) {
          openaiWs.send(
            JSON.stringify({
              type: "conversation.item.create",
              item: {
                ...
                content: [
                  {
                    type: "input_text",
                    text: `[SYSTEM_CONTEXT]: ${closingHint}`,
                  },
                ],
              },
            }),
        ...
        console.error("❌ forceCreateTicket error:", err.message);
        if (openaiWs?.readyState === WebSocket.OPEN) {
          openaiWs.send(
            JSON.stringify({
              type: "conversation.item.create",
              item: {
                ...
                content: [
                  {
                    type: "input_text",
                    text: `[SYSTEM_CONTEXT]: Ticket creation encountered an error: ${err.message}. Apologise and suggest calling 1300 101 414.`,
                  },
                ],
              },
            }),
        ...
        case "conversation.item.input_audio_transcription.completed": {
          ...
          if (pendingTicketConfirmation) {
            ...
            if (ticketConfResult === "yes") {
              ...
              // ── FIX: FORCE CREATE TICKET DIRECTLY ─────────────────
              // Instead of relying on the LLM to call create_ticket,
              // we execute it programmatically and feed the result back
              // so the LLM just delivers the closing message.
              forceCreateTicket();
              break;
            } else if (ticketConfResult === "no") {
              pendingTicketConfirmation = false;
          ...
          if (
            ...
            if (confirmationResult === "yes") {
              ...
              // ── FIX: CHECK IF ALL DETAILS GATHERED → AUTO-TRIGGER ──
              // After email confirmed, if salesStep advanced to confirmTicket
              // or createTicket, immediately trigger the next step so the
              // flow doesn't stall waiting for user input.
              const c = session.collected || {};
              const allDetailsGathered =
                (c._firstName || c.name || c.preferredName) &&
                c.phone &&
                c.email &&
                c._emailStepComplete;

              if (salesStep === "confirmTicket" && allDetailsGathered) {
                // All details collected — tell LLM to ask for confirmation NOW
                const nextStepHint = buildSalesStepHint();
                scheduleResponseCreate(
                  `Email confirmed and saved as "${confirmedEmail}". ` +
                    `_emailStepComplete=true. Do NOT call extract_call_fields with this email again. ` +
                    `Do NOT ask about email again. ALL details are now collected. ` +
                    `${nextStepHint}`,
                  0,
                  true,
                );
                ...
                // Everything confirmed — force create ticket immediately
                forceCreateTicket();
              } else {
                const nextStepHint =
                  buildSalesStepHint() || "Proceed to the next step.";
    ...
    socket.on("structured_input", (payload) => {
      ...
      if (field === "email") {
        ...
        if (openaiWs?.readyState === WebSocket.OPEN) {
          ...
          // ── FIX: After structured email input, check if all details gathered ──
          const c = session.collected || {};
          const allDetailsGathered =
            (c._firstName || c.name || c.preferredName) &&
            c.phone &&
          ...
          let salesHint = buildSalesStepHint() || "";

          if (salesStep === "confirmTicket" && allDetailsGathered) {
            const hint =
              `Customer email confirmed via typed input: ${parsedEmail}. _emailStepComplete=true. ` +
              `Do NOT ask about email again. Do NOT call extract_call_fields with email. ` +
              `ALL details are now collected. ${salesHint}`;
            openaiWs.send(
              JSON.stringify({
                type: "conversation.item.create",
                item: {
